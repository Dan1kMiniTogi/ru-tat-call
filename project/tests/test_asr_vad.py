...
def test_energy_gate_drops_silence() -> None:
    ...
    assert name == "energy"
    assert gate.feed(b"\x00" * WINDOW_BYTES * 20) == 0
...
def test_silero_backend_loads_or_falls_back() -> None:
    ...
    assert gate.feed(b"\x00" * WINDOW_BYTES * 8) == 0
    if name == "silero":
        n = gate.feed(harmonic_pcm(WINDOW_BYTES * 8))
        assert n == WINDOW_BYTES * 8
    else:
        assert gate.feed(sine_pcm(WINDOW_BYTES)) == WINDOW_BYTES
